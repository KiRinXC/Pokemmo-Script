...
class MoveMouse:
    def __init__(self):
        with open('config/RegMouse.json', 'r', encoding="utf-8") as data:
            RegMouse = json.load(data)
            # config.json中每个区域的范围
            self.win_reg = RegMouse['win_reg']['data']
            self.bat_reg = RegMouse['bat_reg']['data']
            self.copy_reg = RegMouse['copy_reg']['data']
            self.esc_reg = RegMouse['esc_reg']['data']
            self.no_reg = RegMouse['no_reg']['data']
            self.pokeinfo_reg = RegMouse['pokeinfo_reg']['data']
            self.item_reg = RegMouse['item_reg']['data']
            self.reflush_reg = RegMouse['reflush_reg']['data']
            self.bag_reg = RegMouse['bag_reg']['data']
            self.skill_reg = RegMouse['skill_reg']['data']
            # 游戏窗口相对于屏幕的坐标
    ...
    def pokeinfo_box_move(self)->None:
        ...
        # 逃跑框的真实位置
        x, y = self.loc_add([temp_x, temp_y], self.win_p)
        pyautogui.moveTo(x, y, self.GR.gen_1d([0.5, 1]))
        pyautogui.click()
